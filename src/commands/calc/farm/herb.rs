...
#[async_trait(?Send)]
impl<O: Write> Command<O> for CalcFarmHerbCommand {
    async fn execute(
        &self,
        ...
        O: 'async_trait,
    {
        ...
        context
            .println_fmt(format_args!("Farming level: {}", farming_level))?;
        // Only print magic level if it's being used
        if let Some(magic_level) = magic_level {
            context
                .println_fmt(format_args!("Magic level: {}", magic_level))?;
        }
        context.println(&herb_cfg.to_string())?;
        context.println("")?;
        context.println(
        ...
        let herb_stats: Vec<(Herb, PatchStats)> =
...
impl CalcFarmHerbCommand {
    ...
    async fn calc_total_patch_stats(
        &self,
        farming_level: usize,
        magic_level: Option<usize>,
        ...
        for patch in &herb_cfg.patches {
            ...
            let patch_stats = patch
                .calc_patch_stats(farming_level, magic_level, herb_cfg, herb)
                .await?;
