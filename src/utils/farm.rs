...
impl HerbPatch {
    ...
    async fn calc_price_stats(
        ...
        let grimy_herb_price = WIKI_ITEM_CLIENT
            .get_avg_price(herb.grimy_herb_item_id())
            .await?;
        let seed_price =
            WIKI_ITEM_CLIENT.get_avg_price(herb.seed_item_id()).await?;

        let compost_price = Self::calc_compost_price(herb_cfg).await?;
        // Cost of resurrection spell is cost of runes times the odds of needing
        // to cast it
        let rune_price = (Spell::ResurrectCrops.rune_cost().await? as f64
    ...
    async fn calc_compost_price(
        herb_cfg: &FarmingHerbsConfig,
    ) -> anyhow::Result<usize> {
        let base_cost = match herb_cfg.compost {
            Some(compost) => {
                let price =
                    WIKI_ITEM_CLIENT.get_avg_price(compost.item_id()).await?;
                // If price==None, that means no record of the compost being
                // traded. This is strange but in that case consider it no-cost
                // (maybe they added some untradeable compost to the game?)
